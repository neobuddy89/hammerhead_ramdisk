...
if [ "${DATA}" -eq "1" ]; then
	$BB mount -t f2fs /dev/block/platform/msm_sdcc.1/by-name/userdata /data -o nosuid,nodev;
	DATA_TYPE=1;
fi;
...
if [ "$SYSTEM_TYPE" -eq "1" ] && [ "$DATA_TYPE" -eq "1" ]; then
	$BB cp /sbin/fstab_mount/fstab_data_system.hammerhead /fstab.hammerhead;
else if [ "$SYSTEM_TYPE" -eq "1" ] && [ "$DATA_TYPE" -eq "0" ]; then
	$BB cp /sbin/fstab_mount/fstab_system.hammerhead /fstab.hammerhead;
else if [ "$SYSTEM_TYPE" -eq "0" ] && [ "$DATA_TYPE" -eq "1" ]; then
	$BB cp /sbin/fstab_mount/fstab_data.hammerhead /fstab.hammerhead;
else
	$BB cp /sbin/fstab_mount/fstab.hammerhead /fstab.hammerhead;
fi;
